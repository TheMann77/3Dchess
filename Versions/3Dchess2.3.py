...
def create_board(new_rows, new_columns, new_boards, knights, pawn_layers, new_depth):
    board = []
    ...
    end_game = 0
    taken = {'W': [], 'B': []}
    new_settings = [new_rows, new_columns, new_boards, knights, pawn_layers, new_depth]
    global boards, rows, columns, depth
    boards, rows, columns, depth = new_boards, new_rows, new_columns, new_depth
    return board, kings, turn, back_pieces, mode, history, present, history_pos, end_game, taken, new_settings


board, kings, turn, back_pieces, mode, history, present, history_pos, end_game, taken, new_settings = create_board(8, 8, 3, True, 1, 2)
...
while True:
    ...
    if not end_game:
        if GUI:
            ...
            #new_rows, new_columns, new_boards, knights, pawn_layers, new_depth
            if new_settings[3]:
                    ...
                    if pygame.mouse.get_pressed(3)[0]:
                        ...
                        if not ((240 < y < 270) or (510 < y < 540) or (x > 240)):
                            ...
                            if [a, b, c] in board[move_from[0], move_from[1], move_from[2]].find_moves():
                                move_to = [a, b, c]
                                print(move_from, move_to, x, y)
                                #print(x, y, move_from, move_to)
        ...
        if pygame.mouse.get_pressed(3)[0]:
            ...
            if 250 < x < 315 and 10 < y < 50:
                ...
                board, kings, turn, back_pieces, mode, history, present, history_pos, end_game, taken, new_settings = create_board(*new_settings)
            elif 250 < x < 320 and 650 < y < 720:
                new_settings[3] = True
            elif 320 < x < 390 and 650 < y < 720:
