    ...
    """if len(posmoves) >= 1:
        ...
        for posmove in posmoves:
            ...
            if depth == 1:    
                ...
                return choose_move(board, colours[colours.index(turn)-1], depth-1, 'maxim' if maxmin == 'minim' else 'minim')
            board[posmove[0][0], posmove[0][1], posmove[0][2]] = board[posmove[1][0], posmove[1][1], posmove[1][2]]
            board[posmove[1][0], posmove[1][1], posmove[1][2]] = to
            board[posmove[0][0], posmove[0][1], posmove[0][2]].location = posmove[0]"""
...
while checkmate == [False, False] and stalemate == False:
    ...
    if turn == 'W':
        ...
        if len(posmoves) >= 1:
            ...
            move = random.choice(maxmoves)"""
        start = time.time()
        move = minimax(board, turn, depth, 'maxim')[1]
