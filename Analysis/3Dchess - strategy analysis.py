...
def new_minimax(turn, overall_turn, depth, start_eval, maximising=True, alpha=-1000000, beta=1000000):
    if depth <= 0 or sum(isinstance(x, King) for layer in board for row in layer for x in row) < 2:
        if evaluate(overall_turn) <= start_eval and depth % 2 == 0:
    ...
    for pos_move in pos_moves:
        ...
        value = new_minimax('W' if turn == 'B' else 'B', overall_turn,
                        depth - 1, start_eval if depth <= 0 and depth % 2 == 0 else evaluate(overall_turn), not maximising, alpha, beta)[0]
        if maximising:
            if value > bestValue:
                bestValue = value
                best_move = pos_move
    ...
    for e in range(50):
        if game_over == None:
            ...
            if True:
                ...
                if turn == 'W':
                    [move_from, move_to] = new_minimax(turn, turn, 2, evaluate(turn))[1]
                else:
                    [move_from, move_to] = minimax(turn, turn, 2)[1]
            ...
            evaluation_w = evaluate('W')
            score_log[e].append(evaluation_w)
            if evaluation_w == 0:
                win_log[e].append(0)
            elif evaluation_w > 0:
                win_log[e].append(1)
...
average_scores = [round(sum(x)/len(x), 2) for i, x in enumerate(score_log) if i%2==1]
average_wins = [round(sum(x)/len(x), 2) for i, x in enumerate(win_log) if i%2==1]
...
plt.fill_between([x for x in range(25)], 0, average_wins)
plt.show()

plt.plot(average_scores)
plt.ylim(-5000, 5000)
plt.xlabel("Moves")
plt.ylabel("White point evaluation")
plt.fill_between([x for x in range(25)], 0, average_scores)
plt.show()
