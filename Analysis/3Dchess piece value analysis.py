...
def evaluate(colour, use_table, fixed):
    score = 0
    for a in range(boards):
        for b in range(columns):
            for c in range(rows):
...
for i in range(50, 60, 10):
    ...
    for game_count in range(games):
        ...
        game_record = [0]
        move_count = 1
        while not end_game and move_count < 500:
            if str_output:
                str_board(board)
                print()
            if turn == 'W':
                move = minimax('W', 'W', wdepth, wtables, evaluate('W', wtables, False))[1]
            else:
                move = minimax('B', 'B', bdepth, btables, evaluate('B', btables, False))[1]
            move_from, move_to = move[0], move[1]

            if board[move_to[0], move_to[1], move_to[2]] != '  ':
                if board[move_to[0], move_to[1], move_to[2]].name == 'K':
                    end_game = True
                    wins.append([turn, move_count])
                    print("win")
                    break
...
plotted = False
for wins in tot_wins:
    if wins:
        plotted = True
        plt.plot([(len([1 for a in wins if a[1] <= x and a[0] == 'W']) - len([1 for a in wins if a[1] <= x and a[0] == 'B']))*100/games for x in range(max([max([f[1] for f in g]) for g in tot_wins]))], label=str(i))
plt.xlabel("Number of moves")
plt.ylabel("Net win percentage (%)")
plt.title("Varying pawn values")
if plotted:
    plt.legend()
plt.show()